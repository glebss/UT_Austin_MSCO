...
int main(int argc, char **argv)
{
    ...
    if (n_vals <= 3) {
        ...
        opts.n_threads = std::min(n_vals / 2 - 1, opts.n_threads);
    }
    pthread_barrier_t barrier;
    pthread_barrier_init(&barrier, NULL, opts.n_threads);
    spin_barrier s_barrier(opts.n_threads);
    fill_args(ps_args, opts.n_threads, n_vals, input_vals, output_vals,
        opts.spin, scan_operator, opts.n_loops, &barrier, &s_barrier);
    ...
    if (sequential)  {
        ...
        pthread_t* threads = alloc_threads(opts.n_threads);

        start_threads(threads, opts.n_threads, ps_args, &compute_prefix_sum_thread);
