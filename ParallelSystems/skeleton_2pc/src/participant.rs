...
impl<'a> Participant<'a> {
    ...
    pub fn new(
        ...
        receiver: Receiver<ProtocolMessage>) -> Participant {
    ...
    pub fn protocol(&mut self) {
        ...
        // phase 1
        let msg_from_coordinator = self.receiver.try_recv();
        match msg_from_coordinator {
            Ok(msg) => {
                self.state = ParticipantState::ReceivedP1;
                let opid = msg.opid + 1;
                let res_operation = self.perform_operation();
                if res_operation {
                    let pm = message::ProtocolMessage::generate(message::MessageType::ParticipantVoteCommit,
                        msg.txid.clone(),
                        self.id_str.clone(),
                        opid);
                    ...
                    let pm = message::ProtocolMessage::generate(message::MessageType::ParticipantVoteAbort,
                        msg.txid.clone(),
                        self.id_str.clone(),
                        opid);
                        let _ = self.sender.send(pm);
