...
__global__ void accumulate_centroids(float* data, float* centroids, int* labels,
    ...
    for (int i = 0; i < num_features; ++i) {
        data_shared[threadIdx.x * num_features + i] = data[idx * num_features + i];
    }
    __syncthreads();
    ...
    if (idx < num_points) {
        ...
        for (int j = 0; j < num_features; ++j) {
            atomicAdd(&centroids[label * num_features + j], data_shared[threadIdx.x * num_features + j]);
        }
    }
    __syncthreads();
}
...
__global__ void average_centroids(float* centroids, int* num_labels, int num_centroids, int num_features) {
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    for (int i = 0; i < num_features; ++i) {
        centroids_shared[threadIdx.x * num_features + i] = centroids[idx * num_features + i];
    }
    __syncthreads();
    ...
    if (idx < num_centroids) {
        ...
        for (int i = 0; i < num_features; ++i) {
            centroids_shared[threadIdx.x * num_features + i] /= (float) count;
        }
    }
    __syncthreads();
    for (int i = 0; i < num_features; ++i) {
        centroids[idx * num_features + i] = centroids_shared[threadIdx.x * num_features + i];
    }
...
__global__ void find_min_distance_2d(float* distances, int num_points, int num_centroids, int* min_labels, int* num_labels) {
    __shared__ int min_labels_shared[NUM_THREADS_PTS];
    int idx_x = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx_x < num_points) {
        ...
        min_labels_shared[threadIdx.x] = min_ind;
        atomicAdd(&num_labels[min_ind], 1);
        __syncthreads();
        min_labels[idx_x] = min_labels_shared[threadIdx.x];
    }
...
__global__ void calculate_distances_2d(float* data, float* centroids, float* distances,
    ...
    __shared__ float distances_shared[NUM_THREADS_X * NUM_THREADS_Y];
    int idx_x = blockIdx.x * blockDim.x + threadIdx.x;
    int idx_y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (idx_x < num_points && idx_y < num_centroids) {
        ...
        distances_shared[threadIdx.x * blockDim.y + threadIdx.y] = sqrt(distance);
        __syncthreads();
        distances[idx_x * num_centroids + idx_y] = distances_shared[threadIdx.x * blockDim.y + threadIdx.y];
    }
}
...
__global__ void compare_centroids(float* centroids, float* old_centroids,
    ...
    __shared__ float centroids_shared[2 * NUM_THREADS_X * NUM_THREADS_Y];
    int idx_x = blockIdx.x * blockDim.x + threadIdx.x;
    int idx_y = blockIdx.y * blockDim.y + threadIdx.y;
    centroids_shared[threadIdx.x * blockDim.y + threadIdx.y] = centroids[idx_x * num_features + idx_y];
    centroids_shared[NUM_THREADS_X * NUM_THREADS_Y + threadIdx.x * blockDim.y + threadIdx.y] = old_centroids[idx_x * num_features + idx_y];
    float d = 0.f;
    if (idx_x < num_centroids && idx_y < num_features) {
        d = centroids_shared[threadIdx.x * blockDim.y + threadIdx.y] - 
            centroids_shared[NUM_THREADS_X * NUM_THREADS_Y + threadIdx.x * blockDim.y + threadIdx.y];
        if (abs(d) > *threshold) {
...
return_cuda kmeans_cuda(float* data, float* centroids, int* labels, 
    ...
    float* distances = new float[num_centroids];
    float* distances_device;
    cudaMalloc((void**)&distances_device, num_centroids * sizeof(float));
    ...
    float* data_device;
    cudaMalloc((void**)&data_device, num_points * args->num_features * sizeof(float));
    cudaMemcpy(data_device, data, num_points * args->num_features * sizeof(float), cudaMemcpyHostToDevice);

    float* centroids_device;
    cudaMalloc((void**)&centroids_device, num_centroids * args->num_features * sizeof(float));
    cudaMemcpy(centroids_device, centroids, num_centroids * args->num_features * sizeof(float), cudaMemcpyHostToDevice);

    float* old_centroids_device;
    cudaMalloc((void**)&old_centroids_device, num_centroids * args->num_features * sizeof(float));
    ...
    while (true) {
        ...
        accumulate_centroids<<<(num_points + NUM_THREADS_PTS - 1) / NUM_THREADS_PTS, NUM_THREADS_PTS, NUM_THREADS_PTS * num_features * sizeof(float)>>>
                    (data_device, centroids_device, labels_device, num_points, num_features);
        average_centroids<<<(num_centroids + NUM_THREADS_X - 1) / NUM_THREADS_X, NUM_THREADS_X, NUM_THREADS_X * num_features * sizeof(float)>>>
